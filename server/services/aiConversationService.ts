...
فقط این یک سوال را به زبان محترمانه و صمیمی مطرح بفرمایید: "${remainingQuestionObj.title}" ${remainingQuestionObj.options && remainingQuestionObj.options !== '[]' ? `( گزینه‌ها: ${remainingQuestionObj.options} )` : ''}`
    : quotationCompleted
    ? `=== وضعیت استعلام ===
تمامی اطلاعات مورد نیاز استعلام تکمیل گردید. به مشتری اعلام کنید اطلاعات ثبت شده و جهت محاسبه قیمت نهایی و اعلام استعلام به کارشناس بیمه ارجاع داده شد.`
    : `=== وضعیت استعلام ===
...
function generateFallbackAnswer(
  ...
  if (quotationCompleted) {
    return 'اطلاعات استعلام قیمت شما با موفقیت دریافت گردید. درخواست جهت استعلام قیمت به کارشناس ارجاع شد و به زودی اعلام خواهد شد.';
  }
  if (nextQuestion) {
