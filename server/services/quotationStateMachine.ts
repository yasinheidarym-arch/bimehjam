...
function parseClassification(raw: unknown, correction = false): QuotationClassification | null {
  ...
  if (value.status === 'QUESTION_ABOUT_FIELD') value.status = 'QUESTION_ABOUT_CURRENT_FIELD';
  if (typeof value.status !== 'string' || !QUOTATION_RESPONSE_STATES.includes(value.status as QuotationResponseState)) {
    if (Array.isArray(value.assignments)) value.status = value.asksQuestion === true ? 'QUESTION_ABOUT_CURRENT_FIELD' : correction ? 'CORRECTION' : value.assignments.length ? 'VALID_ANSWER' : 'AMBIGUOUS';
    else return null;
  }
...
function safeClassifierText(value: unknown): string | null {
  if (typeof value !== 'string') return null;
  const text = value.trim();
  if (!text || text.length > 400 || /کد\s*یکتا|ثبت\s*شد|ارسال\s*شد|قیمت\s*قطعی|کمتر\s*از\s*\d+\s*دقیقه/u.test(text)) return null;
  return text;
}
...
export async function advanceQuotationTurn(input: { sessionId: string; sessionStatus?: string; questions: QuotationTurnQuestion[]; answers: Record<string, string>; previous?: QuotationTurnState | null; message: string; recentMessages?: Array<{ senderType: string; content: string }>; model?: QuotationTurnModel; productKnowledge?: string; behaviorContext?: { channel: string; productId?: string | null; categoryId?: string | null; currentPageUrl?: string | null; intent?: string | null; conversationState?: string | null; quotationState?: string | null; currentField?: string | null; messageType?: string | null; userRole?: string | null }; guidanceSelector?: (context: { message: string; question: QuotationTurnQuestion; knowledge: string; source: 'HELP_TEXT' | 'PRODUCT_KNOWLEDGE'; sourceText: string; tone: string; helpText?: string; productKnowledge?: string; allowedOptions?: string[]; behaviorContext?: { channel: string; productId?: string | null; categoryId?: string | null; currentPageUrl?: string | null; intent?: string | null; conversationState?: string | null; quotationState?: string | null; currentField?: string | null; messageType?: string | null; userRole?: string | null } }) => Promise<unknown>; engine?: { active: boolean; title: string; priority: number; config: QuotationResponseEngineConfig } | null }) {
  ...
  // Semantic correction detection belongs to the classifier. The phrase-based
  // hint is used only by the provider-unavailable fallback.
  ...
  if (['VALID_ANSWER', 'ANSWER_AND_QUESTION', 'MULTI_FIELD_ANSWER', 'CORRECTION'].includes(classification.status)) {
  ...
  if (!Object.keys(updates).length && current && classification.status !== 'QUESTION_ABOUT_CURRENT_FIELD') attempts[current.fieldName] = Math.min(100, (attempts[current.fieldName] || 0) + 1);
  ...
  if (current && ['QUESTION_ABOUT_CURRENT_FIELD', 'ANSWER_AND_QUESTION'].includes(classification.status)) {
    const guidance = await resolveQuotationGuidance({
      ...
      tone: config.states.QUESTION_ABOUT_CURRENT_FIELD.tone,
      select: input.guidanceSelector || (async () => ({ passages: [] })),
      behaviorContext: { ...input.behaviorContext, channel: input.behaviorContext?.channel || 'GOFTINO', conversationState: 'QUOTATION', quotationState: input.sessionStatus || 'IN_PROGRESS', currentField: current.fieldName },
    });
