...
export const PREVIOUS_SAME_PAGE_RESPONSE_V3 = 'اگر مایل باشید می‌تونید فرم استعلام آنلاین همین صفحه رو تکمیل کنید و قیمت بگیرید. اگر ترجیح می‌دید، من همینجا چند سؤال کوتاه ازتون می‌پرسم و درخواستتون رو برای کارشناس می‌فرستم.';
export const PREVIOUS_DIFFERENT_PAGE_RESPONSE_V3 = 'اگر مایل باشید می‌تونید فرم استعلام آنلاین رو تکمیل کنید و قیمت بگیرید:\n{{purchaseUrl}}\nاگر ترجیح می‌دید، من همینجا چند سؤال کوتاه ازتون می‌پرسم و درخواستتون رو برای کارشناس می‌فرستم.';
export const PREVIOUS_SAME_PAGE_RESPONSE_V4 = 'اگر مایل باشید می‌توانید فرم استعلام آنلاین همین صفحه را تکمیل کنید و قیمت بگیرید. اگر ترجیح می‌دهید، من همینجا چند سؤال کوتاه می‌پرسم و درخواستون برای کارشناس میفرستم.';
export const PREVIOUS_DIFFERENT_PAGE_RESPONSE_V4 = 'اگر مایل باشید می‌توانید فرم استعلام آنلاین را تکمیل کنید و قیمت بگیرید:\n{{purchaseUrl}}\nاگر ترجیح می‌دهید، من همینجا چند سؤال کوتاه می‌پرسم و درخواستون برای کارشناس میفرستم.';
...
export const DEFAULT_QUOTATION_ROUTING_TEMPLATES: QuotationRoutingTemplates = {
  ...
  samePageResponse: 'بله، بیمه مناسب کار شما «{{productName}}» است. برای دیدن قیمت، پیشنهاد می‌کنم فرم آنلاین همین صفحه را تکمیل کنید؛ فقط کافی است به چند سؤال ساده پاسخ دهید تا قیمت به‌صورت آنلاین محاسبه شود.\n\nاگر هنگام استعلام سؤالی داشتید، من همین‌جا همراه‌تان هستم. اگر هم ترجیح دادید، می‌توانیم سؤال‌ها را همین‌جا مرحله‌به‌مرحله پیش ببریم.',
  differentPageResponse: 'بله، بیمه مناسب کار شما «{{productName}}» است. برای دیدن قیمت، پیشنهاد می‌کنم فرم آنلاین این بیمه را تکمیل کنید؛ فقط کافی است به چند سؤال ساده پاسخ دهید تا قیمت به‌صورت آنلاین محاسبه شود:\n{{purchaseUrl}}\n\nاگر هنگام استعلام سؤالی داشتید، من همین‌جا همراه‌تان هستم. اگر هم ترجیح دادید، می‌توانیم سؤال‌ها را همین‌جا مرحله‌به‌مرحله پیش ببریم.',
